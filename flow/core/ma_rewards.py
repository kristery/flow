...
def min_delay_unscaled(env):
    ...
    cost = []
    for _, edges in env.network.node_mapping:
        ids = []
        for edge in edges:
        ...
        cost.append(time_step * sum((v_top - vel) / v_top) / (len(ids) + eps))
    return cost
    ...
    for _, edges in env.network.node_mapping:
        ...
        for vel in ids:
            try:
                ...
                pass
        time.append(total_wait_time / (total_veh + 1e-6) * gain)
    return time
